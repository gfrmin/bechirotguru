...
israel_shapes_union <- israel_shapes_raw %>% group_by(SEMEL_YISHUV) %>% summarise(geometry = st_union(Shape)) %>% ungroup %>% ms_simplify(0.05, keep_shapes = TRUE)
...
st_write(bechirot_wide_places, "app/bechirot_wide_places.sqlite", append=FALSE)
